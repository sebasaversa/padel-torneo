function buildPlayerSelect(players, playerIndex, roundIndex, matchIndex, role) {
    const options = players.map((player, index) =>
        `<option value="${index}" ${index === playerIndex ? 'selected' : ''}>${player}</option>`
    ).join('');
    return `<select data-player-select data-round="${roundIndex}" data-match="${matchIndex}" data-role="${role}">${options}</select>`;
}
        ...
        const matches = round.matches.map((match, matchIndex) => {
            ...
            const scoreControl = (team, label) => `
                <div class="score-control ${team === 'score1' ? 'team-one' : 'team-two'}">
                    <button type="button" class="score-adjust" aria-label="Bajar puntaje del ${label} equipo en cancha ${match.court}"
                        data-score-adjust data-round="${roundIndex}" data-match="${matchIndex}" data-team="${team}" data-amount="-1">−</button>
                    <input type="number" min="0" max="${gamesPerSet}" class="score-input" placeholder="0" value="${match[team]}" inputmode="numeric"
                        data-score-input data-round="${roundIndex}" data-match="${matchIndex}" data-team="${team}">
            ...
            return `<div class="match ${matchDone ? 'match-done' : ''}">
                ...
                <div class="team team-one"><div class="team-pair">
                    ${buildPlayerSelect(players, match.t1_p1, roundIndex, matchIndex, 't1_p1')}
                    ${buildPlayerSelect(players, match.t1_p2, roundIndex, matchIndex, 't1_p2')}
                </div></div>
                <div class="vs">CONTRA</div>
                <div class="team team-two"><div class="team-pair">
                    ${buildPlayerSelect(players, match.t2_p1, roundIndex, matchIndex, 't2_p1')}
                    ${buildPlayerSelect(players, match.t2_p2, roundIndex, matchIndex, 't2_p2')}
                </div></div>
